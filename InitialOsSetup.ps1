...
function SetEnvironmentVariables {
    param(
        ...
        [string]$InstructorServerIp = $null,
    ...
    $InstructorServerIp = Read-Host "Enter The Instuctor Server Ip"
    [Environment]::SetEnvironmentVariable("DORON_INSTRUCTOR_SERVER_IP",$InstructorServerIp,"User")
...
function UnrealInstallSteps {
    Write-Host "Installing Epic Games Launcher... "
    winget install EpicGames.EpicGamesLauncher
    Read-Host "Open the Launcher. Press Enter to continue"
    Read-Host "Sign In. Press Enter to continue" 
    Read-Host "Click on the Unreal Engine Tab. Press Enter to continue"
    Read-Host "Click on the libary tab. Press Enter to continue"
    Read-Host "Click the + nect to 'Engine Version'. Press Enter to continue"
    Read-Host "Click the number in the box to choose the engine version. Press Enter to continue"
    Read-Host "Click Install. Press Enter to continue"
    Read-Host "Select the options you want. Press Enter to continue"
    Write-Host "Click install."
}

function NvidiaSurroundSteps {
   Read-Host "Hit the windows key and type 'NVIDIA Control Panel'. Press Enter to continue."
   Read-Host "Click 'Configure Surround, PhysX'. Press Enter to continue."
   Read-Host "Click 'Span displays with Surround' and press 'Configure'. Press Enter to Continue."
   Read-Host "Select you desired resolution and refresh rate and then click 'Enable Surround'. Press Enter to continue"
   Write-Host "Close the opened windows."
}
...
function RunSteps($steps) {
    while ($true) {
        ...
        $steps.Keys | ForEach-Object {
            $stepName = ($_ -csplit '(?=[A-Z])' -ne '') -join ' '
            $completionStatus = if ($steps[$_]) { "Done" } else { "Not done" }
            $color = if ($steps[$_]) { "Green" } else { "Red" }
            Write-Host "$index. $stepName ($completionStatus)" -ForegroundColor $color
            $index++
        }
...
# Main Script Code
if ($Run -eq $False) {
    Write-Host "This Script will assist in the setup of a graphics or IWS computer. -run to run."
    exit
}
...
if ($setupType.ToUpper() -eq "G") {
    ...
    $steps = @{
        ...
        UnrealInstallSteps = $false
        NvidiaSurroundSteps = $false
    }
    RunSteps($steps)
} else {
    # IWS
